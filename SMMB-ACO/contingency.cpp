...
//Methode initiation population(matrix of genomatrice de d̩part types, size of the population)
...
int main () {
    ...
    for (int i = 0; i < nb_indiv_geno; ++ i)
        for (int j = 0; j < nb_snp_pheno; ++ j)
        	Mgeno (i, j) = rand()%3;
    cout << "matrice de d̩part g̩notypes : " << Mgeno << endl;

    int nb_indiv_pheno = Mpheno.size1();

    for (int i = 0; i < nb_indiv_pheno; ++ i)
    	Mpheno (i, 0) = rand()%2;
    cout << "matrice de d̩part ph̩notypes : " << Mpheno << endl;
